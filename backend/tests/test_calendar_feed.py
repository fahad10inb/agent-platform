...
from app import db
from app.ics import _fold, build_ics


def test_fold_measures_octets_not_chars_for_arabic():
    # A long Arabic name (each letter ~2 octets) must fold so every physical line
    # stays <= 75 octets, and no multi-byte character is split (unfolding restores
    # the exact input — proof the split landed on a char boundary).
    line = "SUMMARY:" + ("محمد الفلاسي " * 12)  # ~150 chars, well over 75 octets
    folded = _fold(line)
    parts = folded.split("\r\n")
    assert len(parts) > 1  # it actually folded
    for physical in parts:
        assert len(physical.encode("utf-8")) <= 75, physical
    unfolded = parts[0] + "".join(p[1:] for p in parts[1:])  # drop the fold space
    assert unfolded == line  # nothing lost, no char split


def test_fold_leaves_short_ascii_untouched():
    assert _fold("STATUS:CONFIRMED") == "STATUS:CONFIRMED"
...
def test_the_uid_is_stable_so_a_refeed_does_not_duplicate_the_viewing():
    a = build_ics(
        BIZ,
        [{"id": 7, "date": "2026-07-16", "time": "4:00 PM", "patient_name": "Ahmed"}],
        now=NOW,
    )
    b = build_ics(
        BIZ,
...
def test_the_owner_mints_a_feed_url_and_it_serves_their_bookings(client):
    db.save_booking(
        "bright-smile", "2026-07-16", "4:00 PM", "Ahmed", "0501234567", "viewing"
    )
    url = _mint(client)
...
def test_minting_a_token_is_tenant_scoped(client):
    ...
    assert (
        client.post("/manage/bright-smile/calendar-token", headers=VELVET).status_code
        == 403
    )
    assert (
        client.post("/manage/bright-smile/calendar-token", headers=ADMIN).status_code
        == 200
    )
...
def test_add_to_calendar_returns_one_booking_and_is_authed(client):
    ...
    assert (
        client.get(
            f"/manage/bright-smile/bookings/{bid}.ics", headers=VELVET
        ).status_code
        == 403
    )
    assert (
        client.get(
            "/manage/bright-smile/bookings/99999.ics", headers=BRIGHT
        ).status_code
        == 404
    )

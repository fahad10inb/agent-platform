...
def test_captures_when_the_tool_missed_it():
    _seed(
        ...
        ("model", "Our viewing hours are 9-8."),
    )  # note: the model did NOT capture
    assert ensure_lead_captured(RE, "wa-1") is True
    got = [
        ld
        for ld in db.list_leads("skyline-realty")
        if "9876543" in (ld.get("phone") or "")
    ]
    assert got and got[0]["name"] == "Tariq Aziz"


def test_name_is_the_person_not_the_intent_phrase():
    """A real thread says "I'm interested in…" BEFORE "I'm <name>". The net must
    capture the PERSON, not the intent phrase — the regressioned bug stored the
    name "Interested In" for every auto-caught lead."""
    _seed(
        "wa-6",
        ("user", "Hi, I'm interested in a 2-bedroom in Marina"),
        ("user", "I'm Omar Haddad, my number is 0559873333"),
        ("model", "Our viewing hours are 9-8."),  # the model did NOT capture
    )
    assert ensure_lead_captured(RE, "wa-6") is True
    got = [
        ld
        for ld in db.list_leads("skyline-realty")
        if "9873333" in (ld.get("phone") or "")
    ]
    assert got and got[0]["name"] == "Omar Haddad"
...
def test_skips_when_the_caller_already_booked():
    db.save_booking(
        "skyline-realty", "2026-08-01", "2:00 PM", "Sara", "0559876543", "viewing"
    )
...
def test_skips_non_lead_verticals():
    ...
    assert (
        ensure_lead_captured({"id": "skyline-realty", "vertical": "clinic"}, "wa-5")
        is False
    )
